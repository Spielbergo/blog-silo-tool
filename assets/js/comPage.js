...
function generateLinks() {
    ...
    for (let i = 0; i < userKeywords.length; i++) {
        const currentUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i].replace(/\s+/g, '-').toLowerCase()}/`;
        ...
        if (i === 0) {
            const nextUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i + 1].replace(/\s+/g, '-').toLowerCase()}/`;
            ...
            const prevUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i - 1].replace(/\s+/g, '-').toLowerCase()}/`;
            ...
            linksHtml += `
                <div class="silo-links">
                    <p>
                        Related Article: <a href="${nextUrl}">${userKeywords[i - 1]} ${townNameCapitalized}</a><br>
                        Related Article: <a href="${prevUrl}">${userKeywords[i + 1]} ${townNameCapitalized}</a>
                    </p>
...
function copySingleToClipboard(url, userKeywords) {
    const singleText = `Related Article: <a href="${url}">${userKeywords} ${townNameCapitalized}</a>`;
    copyToClipboard(singleText);
}

function copyPairToClipboard(prevUrl, nextUrl, userKeywords) {
    const pairText = `Related Article: <a href="${prevUrl}">${userKeywords} ${townNameCapitalized}</a>\nRelated Article: <a href="${nextUrl}">${userKeywords} ${townNameCapitalized}</a>`;
