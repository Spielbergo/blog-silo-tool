...
function generateLinks() {
    ...
    // const domain = 'https://stg-jenjewell-staging.kinsta.cloud/';    
    ...
    for (let i = 0; i < userKeywords.length; i++) {
        ...
        // Add slash after town name below if there is a category
        ...
        if (i === 0) {
            ...
            linksHtml += `<button class="code-button" onclick="copySingleToClipboard('${nextUrl}', '${userAnchors[i + 1]}', '${targetAnchors[i]}, ${townName}')">Copy Code</button></div>`;
            ...
            linksHtml += `<button class="code-button" onclick="copySingleToClipboard('${prevUrl}', '${userAnchors[i - 1]}', '${targetAnchors[i]}, ${townName}')">Copy Code</button>`;
        } else {
            const prevUrl = `https://${domainName}${townName}/${userKeywords[i - 1].replace(/\s+/g, '-').toLowerCase()}/`;
            const nextUrl = `https://${domainName}${townName}/${userKeywords[i + 1].replace(/\s+/g, '-').toLowerCase()}/`;
...
function copySingleToClipboard(url, userAnchors, targetAnchors) {
    const singleText = `<h2 class="related-article--container">Click here for more information on <a href="https://stg-jenjewell-staging.kinsta.cloud/">${targetAnchors}</a><br>\nRelated Article: <a href="${url}">${userAnchors}</a></h2>`;
    copyToClipboard(singleText);
}

function copyPairToClipboard(prevUrl, nextUrl, prevAnchor, nextAnchor, targetAnchors) {
    const pairText = `<h2 class="related-article--container">Click here for more information on <a href="https://stg-jenjewell-staging.kinsta.cloud/">${targetAnchors}</a><br>\nRelated Article: <a href="${nextUrl}">${nextAnchor}</a><br>\nRelated Article: <a href="${prevUrl}">${prevAnchor}</a></h2>`;
    copyToClipboard(pairText);
}

// function copySingleToClipboard(url, userAnchors) {
//     const singleText = `<p>Related Article: <a href="${url}">${userAnchors}, Southgate, Ontario, Canada</a></p>`;
//     copyToClipboard(singleText);
// }

// function copyPairToClipboard(prevUrl, nextUrl, prevAnchor, nextAnchor) {
//     const pairText = `<p>Related Article: <a href="${nextUrl}">${nextAnchor}, Southgate, Ontario, Canada</a><br>\nRelated Article: <a href="${prevUrl}">${prevAnchor}, Southgate, Ontario, Canada</a></p>`;
//     copyToClipboard(pairText);
// }