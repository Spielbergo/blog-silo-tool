...
function generateLinks() {
    ...
    for (let i = 0; i < userKeywords.length; i++) {
        const currentUrl = `${domainName}${townName}${userKeywords[i].replace(/\s+/g, '-').toLowerCase()}/`;
        ...
        if (i === 0) {
            const nextUrl = `${domainName}${townName}${userKeywords[i + 1].replace(/\s+/g, '-').toLowerCase()}/`;
            ...
            const prevUrl = `${domainName}${townName}${userKeywords[i - 1].replace(/\s+/g, '-').toLowerCase()}/`;
...
function copySingleToClipboard(targetAnchors) {
    const singleText = `Related Website: ${targetAnchors}`;
    copyToClipboard(singleText);
}

function copyPairToClipboard(prevAnchor, nextAnchor) {
    const pairText = `Related Website: ${nextAnchor}\nRelated Website: ${prevAnchor}`;
