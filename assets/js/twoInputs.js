...
function generateLinks() {
    ...
    for (let i = 0; i < userKeywords.length; i++) {
        const currentUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i].replace(/\s+/g, '-').toLowerCase()}-${lastWord}/`;
        ...
        if (i === 0) {
            const nextUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i + 1].replace(/\s+/g, '-').toLowerCase()}-${lastWord}/`;
            ...
            const prevUrl = `https://${domainName}/ontario/${townName}/${userKeywords[i - 1].replace(/\s+/g, '-').toLowerCase()}-${lastWord}/`;
...
function copyPairToClipboard(prevUrl, nextUrl) {
    const pairText = `Related Article: <a href="${prevUrl}">${prevUrl}</a>\nRelated Article: <a href="${nextUrl}">${nextUrl}</a>`;
